...
@section('header', 'Agregar Factura')

@section('slot')
    <div class="max-w-xl">
        <div class="bg-white rounded-xl shadow p-6">

            <form method="POST" action="{{ route('facturacion.store') }}" class="space-y-4" id="form-factura">
                @csrf

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Código alumno</label>
                    <input type="number" name="codigo_alumno" value="{{ old('codigo_alumno') }}"
                ...
                <div>
                    ...
                    <input type="text" name="concepto" value="{{ old('concepto') }}" id="concepto"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        oninput="toggleOtro()">
                    @error('concepto')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <div id="campo-otro" class="{{ old('concepto') === 'OTRO' ? '' : 'hidden' }}">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Especificar concepto</label>
                    <input type="text" name="concepto_otro" value="{{ old('concepto_otro') }}"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Valor</label>
                    <input type="number" name="valor" value="{{ old('valor') }}" step="0.01"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="codigo_concepto" value="{{ old('codigo_concepto') }}"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="centro_costos" value="{{ old('centro_costos') }}"
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
